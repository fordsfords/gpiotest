...
int main()
{
  ...
  for (int i = 0; i < 5; ++i) {
    ...
    int wlen = write(export_fd, "408", 4);  WCHK(wlen == 4);
    close(export_fd);
   
    int dir_fd = open("/sys/class/gpio/gpio408/direction", O_RDWR);  ECHK(dir_fd != -1);
    wlen = write(dir_fd, "in", 3);  ECHK(wlen == 3);
    close(dir_fd);

    int in_val_fd = open("/sys/class/gpio/gpio408/value", O_RDWR);  ECHK(in_val_fd != -1);
    ...
    wlen = write(export_fd, "409", 4);  WCHK(wlen == 4);
    close(export_fd);
   
    dir_fd = open("/sys/class/gpio/gpio409/direction", O_RDWR);  ECHK(dir_fd != -1);
    wlen = write(dir_fd, "out", 4);  ECHK(wlen == 4);
    close(dir_fd);

    int out_val_fd = open("/sys/class/gpio/gpio409/value", O_RDWR);  ECHK(out_val_fd != -1);
    ...
    wlen = write(export_fd, "408", 4);  ECHK(wlen == 4);
    close(export_fd);
    ...
    /* Release XIO-P1 */
    export_fd = open("/sys/class/gpio/unexport", O_WRONLY);  ECHK(export_fd != -1);
    wlen = write(export_fd, "409", 4);  ECHK(wlen == 4);
    close(export_fd);
  }  /* for i */
